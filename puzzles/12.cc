...
std::vector<int> split(std::string const &str, char sep)
{
    ...
    if (pos != 0)
        out.push_back(std::stoi(str.substr(pos)));
...
void read_input_and_solve()
{
    ...
    while (std::getline(std::cin, line))
    {
        ...
        for (int i = 0; i < 4; i++)
        {
            ...
            for (int j = 0; j < record_original_size; j++)
...
int_fast64_t solve(int c_i, int r_i, int n_s)
{
    if (c_i >= (int)condition.size())
    {
        if (r_i >= (int)record.size())
            ...
            return r_i == (int)record.size() - 1 and n_s == record[r_i];
    }

    if (r_i >= (int)record.size())
    {
        for (; c_i < (int)condition.size(); c_i++)
            if (condition[c_i] == '#')
                return 0;
        return 1;
    }
    ...
    if (c_i == 0 and r_i == 0 and n_s == 0)
        solved_backups.clear();
    ...
    {
    case '?':
        if (solved_backups.contains(key))
            return solved_backups.at(key);
        option_damaged = solve(c_i + 1, r_i, n_s + 1);
        option_operational = n_s == record[r_i] ? solve(c_i + 1, r_i + 1, 0) : !n_s ? solve(c_i + 1, r_i, 0)
                                                                                    : 0;
        solved_backups.insert(std::make_pair(key, option_damaged + option_operational));
        return option_damaged + option_operational;
    case '.':
        if (!n_s)
            return solve(c_i + 1, r_i, 0);
        else if (n_s != record[r_i])
            return 0;
        else
            return solve(c_i + 1, r_i + 1, 0);
    case '#':
        if (n_s + 1 > record[r_i])
            return 0;
        else
            return solve(c_i + 1, r_i, n_s + 1);
    }
